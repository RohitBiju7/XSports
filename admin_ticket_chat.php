...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['message'])) {
    ...
    if (empty($message)) {
        ...
        $error_message = "This ticket is pending. Please accept it first.";
    } else {
        try {
    ...
    switch ($_POST['action']) {
        case 'accept':
            ...
            if ($stmt->execute([$ticket_id])) {
                $success_message = "Ticket accepted successfully!";
                // Refresh ticket data
                $stmt = $pdo->prepare("SELECT t.*, u.name as user_name, u.email as user_email 
                                      FROM support_tickets t 
                                      JOIN users u ON t.user_id = u.id 
                ...
                $error_message = "Error accepting ticket.";
            }
            break;
        ...
        case 'resolve':
            ...
            if ($stmt->execute([$ticket_id])) {
                $success_message = "Ticket marked as resolved!";
                // Refresh ticket data
                $stmt = $pdo->prepare("SELECT t.*, u.name as user_name, u.email as user_email 
                                      FROM support_tickets t 
                                      JOIN users u ON t.user_id = u.id 
...
<head>
    ...
    <title>Ticket Chat - XSports</title>
...
<body>
    ...
    <div class="admin-container">
        <div class="chat-header">
            <a href="admin_support.php" class="back-button">
                <i class="fa-solid fa-arrow-left"></i> Back
            </a>
            <h1><i class="fa-solid fa-comments"></i> Ticket Chat</h1>
        ...
        <div class="send-message-section">
            ...
            <form method="POST" class="message-form">
                <div class="form-group">
                    ...
                    <textarea id="message" name="message" placeholder="Type your message here..." required><?php echo htmlspecialchars($_POST['message'] ?? ''); ?></textarea>
                </div>
                
                <div class="form-actions">
                    <button type="submit" class="btn-send" <?php echo ($ticket['status'] === 'resolved' || $ticket['status'] === 'pending') ? 'disabled' : ''; ?>>
                    ...
                    <?php elseif ($ticket['status'] === 'pending'): ?>
                        <span style="color: #ffc107; font-size: 0.9rem;">
                            <i class="fa-solid fa-clock"></i> Please accept this ticket before sending messages.
                        </span>
                    <?php endif; ?>
                </div>
            </form>
